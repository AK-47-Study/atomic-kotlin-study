...
`import` 키워드를 사용해 다른 파일에 정의된 코드를 `재사용`할 수 있다.

`import`를 쓰는 방법 중 하나는 클래스, 함수, 프로퍼티 이름을 지정.

패키지는 연관 있는 코드를 모아둔 것이다.

`as` 키워드를 사용해 `임포트`하면서 이름을 변경할 수 있다.
...
1. `vararg`로 선언된 파라미터 위치의 인자에 `Array`를 넘길 수 있다.
2. `Array`를 만들려면 `listOf()`와 마찬가지 방법으로 `arrayOf()`를 호출한다.
3. `Array`는 항상 가변이란 점 유의!
4. `Array`를 여러 값으로 이뤄진 시퀀스로 취급하려면 `스프레드 연산자` 인 * 를 써야 한다.
5. 스프레드 연산자는 `배열`에만 적용할 수 있다.
6. `List`가 있고 `vararg` 인자 부분에 넘기고 싶다면 `Array`로 변환 후 `스프레드 연산자`를 적용해야한다.
7. 원시 타입의 배열을 사용해야므로 `toIntArray()`처럼 구체적인 원시 타입의 배열을 돌려주는 함수를 호출해야한다.

### 11. 집합

> `Set`은 값이 한 번 씩만 들어갈 수 있는 컬렉션이다. `Set`은 중복을 `자동으로 제거`해준다.
>
...
fun main() {
	...
	colors eq
		...
		listOf(Blue11, "Green", "Green", "Yellow")
...
1. **ListOfStrings.kt**에서 설명했듯이 **split()**과 정규식을 사용해 **String**을 분할한다
2. **`colors`**를 읽기 전용 **`Set**인 **colorSet**`에 복사해 넣으면 중복되는 **`Green`**문자열 중 하나가 제거된다.
3. 여기서는 + 연산을 사용해 새로운 **`Set`**을 만들고 표시한다. 중복된 원소를 **`Set`**에 추가하면 하나만 남고 중복이 사라진다.
4. **toM**니**tableSet()**은 읽기 전용 **Set**에서 새 **Set**을 만든다.
5. **`MutableList`**에서와 마찬가지로, **`MutableSet`**에서도 `+=과-=이` 집합에 원소들을 추가하거나 삭제한다.
6. **`in`**과 **`contains()`** 를 사용해 집합의 원소인지 검사할 수 있다.

합집합, 교집합, 차집합 등의 일반적인 수학 집합 연산도 모두 제공한다.

### 12. `프로퍼티 접근자`

> `프로퍼티 접근자`를 `정의`함으로써 `프로퍼티`를 읽고 쓸 때 일어나는 동작을 변경할 수 있다.
>

`프로퍼티`의 값을 읽을 때 사용하는 접근자를 게터라고 한다.

게터를 생성하려면 `get()`를 프로퍼티 선언 바로 뒤에 정의해야한다.
